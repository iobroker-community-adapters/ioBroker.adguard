...
async function intervalTick(adapter, pollInterval) {
    ...
    try {
        ...
        stats.ratio_blocked_filtering =
            Math.round((stats.num_blocked_filtering /
        ...
        stats.ratio_replaced_safebrowsing =
            Math.round((stats.num_replaced_safebrowsing /
        ...
        stats.ratio_replaced_parental =
            Math.round((stats.num_replaced_parental /
        ...
        for (const key in control) {
            ...
            if (key == 'adguard_protection') {
                await setObjectAndState(adapter, `control.${key}`, `control.${key}`, null, control[key].protection_enabled);
            }
            else {
...
async function setObjectAndState(adapter, objectId, stateId, stateName, 
